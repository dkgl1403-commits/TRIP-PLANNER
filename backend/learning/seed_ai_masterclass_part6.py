        ...
        config = {
            "parts": [
                ...
                },
                {
                    ...
                    "readingTime": "~2 min read",
                    "narrative": "<p>So, we know AI needs to output probabilities. But when a Neural Network finally makes a prediction, what does that prediction actually look like internally?</p><p>If you show an AI a picture and ask, \"Is this a Cat, a Dog, or a Bird?\", the AI's final layer of neurons does not magically output \"99% Cat\". Instead, it outputs raw, messy mathematical numbers called <strong>Logits</strong>.</p><p><strong>But where do Logits come from?</strong> Remember the thousands of internal 'Weights' (volume knobs) we talked about earlier? When you input an image of a Cat, those pixels travel through layers of math. Each layer multiplies the inputs by its weights. By the time it reaches the very last layer of the network, all that math has been summed up into a final raw score for each possible category.</p><p>That final, unrefined mathematical sum is the Logit. It might output <code>2.5</code> for Cat (because the 'whiskers' and 'pointy ears' weights fired strongly), <code>-1.0</code> for Dog, and <code>0.8</code> for Bird. How do we translate these messy Logits into clean percentages? We use a mathematical function called <strong>Softmax</strong>.</p>",
                    "audioText": "When an AI makes a prediction, its internal neurons don't output clean percentages. They output raw, messy numbers called Logits. These logits are the final sum of all the math and weights inside the network. To turn these messy sums into human readable percentages, it uses a mathematical function called Softmax.",
                    "audioTextHinglish": "Jab AI prediction karta hai, toh uske neurons direct percentages nahi dete. Woh raw aur messy numbers output karte hain jinhe Logits kehte hain. Yeh logits network ke andar ke saare weights aur math ka final sum hote hain. Inhe percentages mein badalne ke liye Softmax function use hota hai.",
                    "keyInsight": "Logits are the raw, unfiltered mathematical sums of the Neural Network.",
                    "widgetType": None,
                    "widgetData": {}
                },
                {
