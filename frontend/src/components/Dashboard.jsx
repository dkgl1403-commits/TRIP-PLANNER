...
function Dashboard({ user, onLogout, theme, toggleTheme, onCreateTrip, onAiPlanTrip, onViewTrip, onAdminDashboard, onFinanceDashboard, onSystemHealth }) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col pt-24 px-4 sm:px-8 max-w-container-max mx-auto text-on-surface font-body-md">
      {/* Navigation Bar */}
      <nav className="fixed top-0 left-0 right-0 h-20 px-8 flex items-center justify-between bg-glass-fill backdrop-blur-[24px] border-b border-glass-stroke z-50">
        ...
        <div className="hidden md:flex h-full items-center gap-8">
          <button 
            className={`font-title-md text-lg h-full border-b-2 transition-colors ${activeTab === 'dashboard' ? 'border-neon-coral text-neon-coral' : 'border-transparent text-on-surface-variant hover:text-on-surface'}`} 
            onClick={() => setActiveTab('dashboard')}
          >
            Dashboard
          </button>
          <button 
            className={`font-title-md text-lg h-full border-b-2 transition-colors ${activeTab === 'mytrips' ? 'border-neon-coral text-neon-coral' : 'border-transparent text-on-surface-variant hover:text-on-surface'}`} 
            onClick={() => setActiveTab('mytrips')}
          >
            My Trips
          </button>
          <button 
            className={`font-title-md text-lg h-full border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'locations' ? 'border-neon-coral text-neon-coral' : 'border-transparent text-on-surface-variant hover:text-on-surface'}`} 
          ...
          {isProfileOpen && (
            <div className="absolute top-14 right-0 w-64 bg-surface-container-high border border-glass-stroke rounded-xl shadow-2xl py-2 z-50 overflow-hidden">
              ...
              <div className="py-2">
                ...
                {user?.role === 'ADMIN' && (
                  <>
                    <button className="w-full px-4 py-2 text-left flex items-center gap-3 text-on-surface-variant hover:text-on-surface hover:bg-surface-variant transition-colors" onClick={onSystemHealth}>
                      <span className="material-symbols-outlined text-[20px]">health_and_safety</span> System Health
                    </button>
                ...
                {(user?.role === 'ADMIN' || user?.role === 'FINANCE_USER') && (
                  <button className="w-full px-4 py-2 text-left flex items-center gap-3 text-on-surface-variant hover:text-on-surface hover:bg-surface-variant transition-colors" onClick={onFinanceDashboard}>
                    <span className="material-symbols-outlined text-[20px]">monitoring</span> Finance Dashboard
                  </button>
                )}
                {isBiometricEnabled && (
                  <button className="w-full px-4 py-2 text-left flex items-center gap-3 text-on-surface-variant hover:text-on-surface hover:bg-surface-variant transition-colors" onClick={async () => {
                    ...
                    <span className="material-symbols-outlined text-[20px]">fingerprint</span> Disable Biometrics
                  </button>
                )}
              </div>
